...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    @Autowired
    public CustomOAuth2UserService(MyUserService myUserService) {
...
//        System.out.println(response.getStatusCode());
        ...
        multiPrincipal(user, token);

        return user;
    }

    private void multiPrincipal(DefaultOAuth2User user, OAuth2AccessToken token){
        SecurityContext context = SecurityContextHolder.getContext();
        Authentication authentication = context.getAuthentication();
        if(authentication == null){
            myUserService.oauth2MyUserVk(user, token);
            return;
        }
        myUserService.connectVkAccount((DefaultOAuth2User) authentication.getPrincipal(), user, token);
